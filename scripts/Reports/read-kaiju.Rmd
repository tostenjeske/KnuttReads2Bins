...
if(exists("snakemake")){
  ...
  alignedresults.file <- "/run/user/2496/gvfs/sftp:host=highmem1/data1/2019_LJ_data/bachelor_thesis/metadiet/output/Analysis/read_classification/kaiju/kaiju_classification_results.tsv.gz"
...
alignedresults <- fread(alignedresults.file,select = c("sample","lengthorscore","taxid","tax","Classified","accession_matches"),sep="\t",fill=T)
alignedresults.counts <- alignedresults[,.N,by=c("sample","Classified")]
alignedresults <- alignedresults[Classified=="C",]
alignedresults[,accession_matches:=str_count(accession_matches,fixed(","))]
...
```{r}
dat = alignedresults[,.N,by=c("sample","accession_matches")]
dat[,rel:=N/sum(N)*100,by=sample]
ggplotly(ggplot(dat) + aes(x=accession_matches,y=rel,color=sample) + geom_line() + scale_color_brewer(palette  = sample_palette)+ xlab("Number of References used") + ylab("% of Classified\n Reads") + labs(color="Sample") + scale_y_continuous(labels = function(x) paste0(x, "%")))
```
