...
merge_inserts<-fread(merge_inserts.file,sep="\t",fill=T)
merge_inserts<-merge_inserts[insert!=-1,density(insert)[c("x","y")],by="sample"]

sampleoverview_merge <- fread(sampleoverview_merge.file)
sampleoverview_merge = fastqcreports.tr[sampleoverview_merge,on=c("sample","readdirection")]
tplot_merge <- fread(tplot_merge.file)


sampleoverview_merge_untr <- fread(sampleoverview_merge_untr.file)
sampleoverview_merge_untr = fastqcreports.utr[sampleoverview_merge_untr,on=c("sample","readdirection")]
tplot_merge_untr <- fread(tplot_merge_untr.file)
...
```{r}
sampleoverview.vis = sampleoverview[,.(sample,trimmed,readdirection,file,qcreport,minReadLen,maxReadLen,reads)]
colnam = c(sample="Sample",trimmed="Trimmed",readdirection="Group",qcreport="FASTQC Report",minReadLen="Shortest Read [bp]",maxReadLen="Longest Read [bp]",reads="Number of read pairs")
...
```{r}
ggplot(sampleoverview) + aes(x=readdirection,lower=quantile25ReadLen,upper=quantile75ReadLen,middle=medianReadLen,ymin=minReadLen,ymax=maxReadLen,group=interaction(sample,trimmed,readdirection),fill=sample) + geom_boxplot(stat="identity",position = "dodge")+scale_fill_brewer(palette = sample_palette) + xlab ("Read group") +ylab("Read length in bp") + facet_wrap(~trimmed) + labs(fill="Sample") + theme(legend.position = "bottom")
```
...
ggplotly(ggplot(dat) + aes(x=x,y=y,linetype=readdirection,color=sample)  + geom_line() +scale_color_brewer(palette = sample_palette)+ xlab("Read Length in bp") + facet_wrap(~trimmed) +  ylab("Density Estimate") +theme(legend.title = element_blank())) 
```
...
#dat = tplot[type=="gccontent_density" & x>=0 & x<=1,]
ggplotly(ggplot(dat) + aes(x=x,y=y,linetype=readdirection,color=sample)  + geom_line() +scale_color_brewer(palette = sample_palette)+ xlab("Average Phred Score for a whole read") + ylab("Density Estimate") +theme(legend.title = element_blank()) + facet_wrap(~trimmed))
```
...
```{r}
dat = tplot[type=="cycle_qual_counts",.(median=weightedMedian(y,z)),by=c("sample","readdirection","x","trimmed")]
ggplotly(ggplot(dat) + aes(x=x,y=median,color=sample,linetype=readdirection) +geom_line()+scale_color_brewer(palette = sample_palette)+ ylab( "Median phred score")+xlab("Cycle")+theme(legend.title = element_blank())+ facet_wrap(~trimmed))
```
