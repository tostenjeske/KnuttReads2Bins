...
ggplotly(ggplot(bps) + aes(x=sample, y=bpscut,fill=Read) + scale_fill_brewer(palette = read_palette) + geom_bar(stat="identity",position = "dodge") + xlab("Sample") + scale_y_continuous(labels = function(x) paste0(x, "‰")) + ylab("Basepairs cut"))
```
...
```{r}
ggplotly(ggplot(tplot) + aes(x=x,y=median,color=sample,linetype=read) +geom_line()+scale_color_brewer(palette = sample_palette)+ ylab( "Median phred score")+xlab("Cycle") + theme(legend.title = element_blank()) )
```
