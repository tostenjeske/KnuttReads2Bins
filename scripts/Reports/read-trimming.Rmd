...
if(exists("snakemake")){
  ...
  trim_comparison.file <- snakemake@input[["trim_summary_beforeafter"]]
...
files = c(merge_untrimmed.file,merge_trimmed.file,trim_summary.file,trim_details.file,trim_comparison.file,sampleoverview_trim.file,tplot_raw.file,tplot_trimmed.file)

source(commonscript.file)
commonOptions(threads)
suppressPackageStartupMessages(require(matrixStats))

merge_untrimmed<-fread(merge_untrimmed.file)
merge_untrimmed[,adaptercount:=round(adaptercount/pairstomerge*100,2)]
merge_trimmed<-fread(merge_trimmed.file)
merge_trimmed[,adaptercount:=round(adaptercount/pairstomerge*100,2)]

trim_comparison = fread(trim_comparison.file)

sampleoverview_trim = fread(sampleoverview_trim.file)
sampleoverview_raw = fread(sampleoverview_raw.file)

tplot_raw = fread(tplot_raw.file)
tplot_trimmed = fread(tplot_raw.file)

tplot = rbind(tplot_raw[type=="cycle_qual_counts",.(trimmed="trimmed",median=weightedMedian(y,z)),by=c("sample","readdirection","x")],tplot_trimmed[type=="cycle_qual_counts",.(trimmed="untrimmed",median=weightedMedian(y,z)),by=c("sample","readdirection","x")])
tplot[,readdirection:=paste0(trimmed,".",readdirection)]

bpin_info <- sampleoverview_raw[,c("sample","readdirection","basepairs")]
bpin_info <- dcast(bpin_info,sample~readdirection,value.var = "basepairs")
colnames(bpin_info) <- c("sample","in1_bp","in2_bp")

trim_summary<-fread(trim_summary.file)
...
ggplotly(ggplot(data, aes(x=sample, y=value,fill=sample)) + geom_bar(stat="identity") + xlab ("Sample") + ylab("Discarded Read pairs") + labs(fill = "Sample") + theme(legend.position = "none") + scale_color_brewer(palette = sample_palette)+ scale_y_continuous(labels = function(x) paste0(x, "%")))
```
...
ggplotly(ggplot(bps) + aes(x=sample, y=bpscut,fill=Read) + scale_fill_brewer(palette = readdirection_palette) + geom_bar(stat="identity",position = "dodge") + xlab("Sample") + scale_y_continuous(labels = function(x) paste0(x, "‰")) + ylab("Basepairs cut"))
```
...
```{r}
ggplot(trim_comparison) + aes(x=readdirection,lower=quantile25ReadLenChange,upper=quantile75ReadLenChange,middle=medianReadLenChange,ymin=minReadLenChange,ymax=maxReadLenChange,group=interaction(sample,readdirection),fill=sample) + geom_boxplot(stat="identity",position = "dodge")+scale_fill_brewer(palette = sample_palette) + xlab ("Read group") +ylab("Read length in bp") + labs(fill="Sample") + theme(legend.position = "bottom")
```
...
```{r}
ggplot(trim_comparison) + aes(x=readdirection,lower=quantile25AvgQualChange,upper=quantile75AvgQualChange,middle=medianAvgQualChange,ymin=minAvgQualChange,ymax=maxAvgQualChange,group=interaction(sample,readdirection),fill=sample) + geom_boxplot(stat="identity",position = "dodge")+scale_fill_brewer(palette = sample_palette) + xlab ("Read group") +ylab("Average Sequence Quality change") + labs(fill="Sample") + theme(legend.position = "bottom")
```
...
```{r}
ggplotly(ggplot(tplot) + aes(x=x,y=median,color=sample,linetype=readdirection) +geom_line()+scale_color_brewer(palette = sample_palette)+ ylab( "Median phred score")+xlab("Cycle") + theme(legend.title = element_blank()) )
```
...
```{r}
sampleoverview_trim.vis = sampleoverview_trim[,.(sample,readdirection,minReadLen,maxReadLen,reads)]
sampleoverview_trim.vis = fastqcreports[sampleoverview_trim.vis,on=c("sample","readdirection")]
